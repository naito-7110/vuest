...
describe('@vuest/core#Toggle', async () => {
  test.each<{
    props: { pressed?: boolean; loading?: boolean }
    expectedPressed: string
    expectedBusy: string | undefined
    expectedDisabled: string | undefined
  }>([
    { props: {}, expectedPressed: 'false', expectedBusy: undefined, expectedDisabled: undefined },
    {
      props: { pressed: true },
      expectedPressed: 'true',
      expectedBusy: undefined,
    ...
    {
      props: { pressed: true, loading: true },
      expectedPressed: 'true',
      expectedBusy: 'true',
      expectedDisabled: 'true',
    },
  ])(
    'initial state with props: $props',
    async ({ props, expectedPressed, expectedBusy, expectedDisabled }) => {
  ...
  test.each([
    ...
    'click behavior with loading=$loading, pressed=$initialPressed',
    async ({ loading, initialPressed, expectedAfterClick }) => {
